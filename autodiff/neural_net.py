...
class Neuron(Module):
    ...
    def __call__(self, x: List[Scalar]) -> Scalar:
        ...
        output = Scalar(0)
        for i in range(len(self.w)): output += self.w[i] * x[i]
        output += self.b
        if self.use_relu:
            output = output.relu()
        return output
...
class FeedForwardLayer(Module):
    def __init__(self, num_inputs: int, num_outputs: int, use_relu: bool):
        ...
        for n in range(num_outputs): 
            self.neurons.append(Neuron(num_inputs=num_inputs, use_relu=use_relu))
...
class MultiLayerPerceptron(Module):
    def __init__(self, num_inputs: int, num_hidden: List[int], num_outputs: int):
        ...
        before = num_inputs
        for n in num_hidden:
            self.layers.append(FeedForwardLayer(before, n, True))
            before = n
        self.layers.append(FeedForwardLayer(before, num_outputs, False))
